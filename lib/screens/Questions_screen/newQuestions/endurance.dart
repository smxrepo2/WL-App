...
import 'gymRoutine.dart';

class EnduranceQuestion extends StatefulWidget {
  ...
  @override
  State<EnduranceQuestion> createState() => _EnduranceQuestionState();
}

class _EnduranceQuestionState extends State<EnduranceQuestion> {
  ...
  void initState() {
    ...
    _questoins = _questionsModel.questoins[13];
    ...
    return Scaffold(
      body: Column(
        children: [
                        ...
                        setState(() {
                          ...
                          if (selectedOption == -1) {
                            ...
                            var user = Provider.of<UserDataProvider>(context,
                                listen: false);
                            String endurance = data[selectedOption];
                            int userId = user.userData.user.id;

                            AddEnduranceQuestionRequestModel requestModel =
                                AddEnduranceQuestionRequestModel(
                                    UserId: userId,
                                    MinExercise: endurance,
                                    QuestionOrder: 14);
                            addEndurance(requestModel).then((value) {
                              if (value != null) {
                                if (value.response ==
                                    "Data updated successfully") {}
                                AuthService.setQuestionOrder(14);
              ...
              if (selectedOption == -1) {
                ...
                showDialog(
                    ...
                    builder: (context) {
                      return AlertDialog(
                        content: const Text(
                            'Please select any option to continue further'),
                        actions: [
                          TextButton(
                              onPressed: () {
                                Navigator.pop(context);
                ...
                addEndurance(requestModel).then((value) {
                  if (value != null) {
                    ...
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => GymRoutineQuestion(
          ...
          // countingText(
          //     number: 14, totalQuestion: _questionsModel.questoins.length),
  ...
  Future<AddEnduranceQuestionResponseModel> addEndurance(
    ...
    try {
      ...
      AddEnduranceQuestionResponseModel response =
          await _questionRepository.addEndurance(requestModel);
      EasyLoading.dismiss();
      return response;
    } catch (e) {
      // Get.showSnackbar();
