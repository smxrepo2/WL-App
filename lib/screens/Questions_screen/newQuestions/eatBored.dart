...
class EatBoredQuestion extends StatefulWidget {
  ...
  @override
  State<EatBoredQuestion> createState() => _EatBoredQuestionState();
}

class _EatBoredQuestionState extends State<EatBoredQuestion> {
  ...
  void initState() {
    ...
    _questoins = _questionsModel.questoins[20];
    ...
    return Scaffold(
      body: Column(
        children: [
                        ...
                        setState(() {
                          ...
                          if (selectedOption == -1) {
                            ...
                            var user = Provider.of<UserDataProvider>(context,
                                listen: false);
                            String eatBored = data[selectedOption];
                            int userId = user.userData.user.id;

                            AddEatBoredQuestionRequestModel requestModel =
                                AddEatBoredQuestionRequestModel(
                                    UserId: userId,
                                    BoredEating: eatBored,
                                    QuestionOrder: 21);
                            addEatBored(requestModel).then((value) {
                              if (value != null) {
                                if (value.response ==
                                    "Data updated successfully") {
                                  AuthService.setQuestionOrder(21);
                                  //Todo: Testing
                                  Navigator.push(
                                      context,
                                      MaterialPageRoute(
              ...
              if (selectedOption == -1) {
                ...
                showDialog(
                    ...
                    builder: (context) {
                      return AlertDialog(
                        content: const Text(
                            'Please choose any option to proceed further'),
                        actions: [
                          TextButton(
                              onPressed: () {
                                Navigator.pop(context);
                  ...
                  if (value != null) {
                    if (value.response == "Data updated successfully") {
                      ...
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => EatQuickerQuestion(
          ...
          // countingText(
          //     number: 21, totalQuestion: _questionsModel.questoins.length),
  ...
  Future<AddEatBoredQuestionResponseModel> addEatBored(
    ...
    try {
      ...
      AddEatBoredQuestionResponseModel response =
          await _questionRepository.addEatBored(requestModel);
      EasyLoading.dismiss();
      return response;
    } catch (e) {
      // Get.showSnackbar();
