...
class PastDietsQuestion extends StatefulWidget {
  ...
  @override
  State<PastDietsQuestion> createState() => _PastDietsQuestionState();
}

class _PastDietsQuestionState extends State<PastDietsQuestion> {
  ...
  void initState() {
    ...
    _questoins = _questionsModel.questoins[18];
    ...
    return Scaffold(
      body: Column(
        children: [
                      ...
                      onTap: () {
                        setState(() {
                          ...
                          if (selectedOption == -1) {
                            ...
                            widget.signupBody.mindQuestions.cravings =
                                data[selectedOption];
                            ...
                            var user = Provider.of<UserDataProvider>(context,
                                listen: false);
                            String pastDiets = data[selectedOption];
                            int userId = user.userData.user.id;

                            AddPastDietsQuestionRequestModel requestModel =
                                AddPastDietsQuestionRequestModel(
                                    UserId: userId,
                                    CraveFoods: pastDiets,
                                    QuestionOrder: 19);
                            addPastDiets(requestModel).then((value) {
                              if (value != null) {
                                if (value.response ==
                                    "Data updated successfully") {
                                  AuthService.setQuestionOrder(19);
                                  //Todo: Testing
                                  Navigator.push(
                                      context,
                                      MaterialPageRoute(
          ...
          GestureDetector(
            onTap: () {
              ...
              if (selectedOption == -1) {
                ...
                widget.signupBody.mindQuestions.cravings = data[selectedOption];
                  ...
                  if (value != null) {
                    if (value.response == "Data updated successfully") {
                      ...
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => EatStressedQuestion(
          ...
          // countingText(
          //     number: 19, totalQuestion: _questionsModel.questoins.length),
  ...
  Future<AddPastDietsQuestionResponseModel> addPastDiets(
    ...
    try {
      ...
      AddPastDietsQuestionResponseModel response =
          await _questionRepository.addPastDiets(requestModel);
      EasyLoading.dismiss();
      return response;
    } catch (e) {
      // Get.showSnackbar();
