...
class NewAddFoodScreen extends StatefulWidget {
  String foodId;

  NewAddFoodScreen(this.foodId);

  @override
  _NewAddFoodScreenState createState() => _NewAddFoodScreenState();
}

class _NewAddFoodScreenState extends State<NewAddFoodScreen> {
  ...
  Future<FoodModel> getFoodDetail() async {
    final response = await get(
      Uri.parse('$apiUrl/api/food/${widget.foodId}'),
      // ${widget.foodId}
    );
    print("response ${response.statusCode} ${response.body}");
    if (response.statusCode == 200) {
  ...
  Future<dynamic> getFoodDetail1() async {
    final response = await get(
      Uri.parse('$apiUrl/api/food/${widget.foodId}'),
      // ${widget.foodId}
    );
                      ...
                      Column(
                        ...
                        children: [
                          Text(
                            ...
                            style: style.copyWith(
                                fontWeight: FontWeight.w600, color: pinkColor),
                          ),
                          SizedBox(
                            height: 5,
                          ),
